...
import HeadPermissionModal from './headPermissionModal';
...
interface addHeadDepartmentModalProps {
  setHeadName: (v: string) => void;
  setShowDialog: (v: boolean) => void;
  setHeadId: (v: number) => void;
  setHeadPermissions?: (p: number[]) => void;
  setIsHeadPermissionOpen?: (c: boolean) => void;
  selectedPermissions?: number[];
  isHeadPermissionOpen?: boolean;
...
export default function AddHeadDepartmentModal({
  setShowDialog,
  setHeadName,
  setHeadId,
  setHeadPermissions,
  setIsHeadPermissionOpen,
  selectedPermissions,
  isHeadPermissionOpen,
  currentHeadId,
  setPermissionChanged,
  permissionChanged,
  currentDeputyId,
  initialSelectedIds,
}: addHeadDepartmentModalProps) {
  const [empList, setEmpList] = useState<Employee[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedId, setSelectedId] = useState<number>(currentHeadId || 0);
    ...
    const fetchData = async () => {
      try {
        ...
        if (res && res.data?.data.employees) {
          ...
          const selectedEmployeeIds = [currentDeputyId, ...initialSelectedIds];
          list = list.filter((emp) => !selectedEmployeeIds.includes(emp.id));
          console.log(list);
          if (currentHeadId) {
            const idx = list.findIndex((m) => m.id === currentHeadId);
            if (idx > -1) {
  ...
  const handleSelect = (member: Employee) => {
    setHeadName(member.fullName);
    setHeadId(member.id);
    setSelectedId(member.id);
  };

  const handleClose = () => {
    if (permissionChanged || isHeadPermissionOpen) {
      const confirmClose = window.confirm(
        t('common.confirmCloseWithoutSaveOrClosePermissionModal'),
      );
      if (!confirmClose) return;
    }
    if (setIsHeadPermissionOpen) setIsHeadPermissionOpen(false);
    setShowDialog(false);
  };
  ...
  return (
    <div className="flex gap-5">
      <div className="inset-0 flex items-center justify-center z-50">
        <div
          className={`bg-white rounded-lg shadow-2xl p-6 h-auto max-h-[85vh] flex flex-col ${isHeadPermissionOpen ? 'w-[30vw]' : 'w-[35vw]'} `}
        >
      ...
      {isHeadPermissionOpen && (
        <HeadPermissionModal
          ...
          onClose={() => setIsHeadPermissionOpen?.(false)}
          onSave={(newPermissions) => {
            setHeadPermissions?.(newPermissions);
            setIsHeadPermissionOpen?.(false);
          }}
          setPermissionChanged={setPermissionChanged}
        />
      )}
    </div>
  );
